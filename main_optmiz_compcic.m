...
% [~,~, alldata] = xlsread('traindatacic.xlsx');

% normalization
% [matrix_normalized,t] = preprocessor(alldata);
load('matrix_normalizedcic.mat')
load('tarcic.mat')
...
opt_matrixtrain_pso(opt_matrixtrain_pso<=0) = 0.2;

% SVM training
svmstrcut = fitcsvm(opt_matrixtrain_pso,tr,'KernelFunction','RBF');

% read test data
[~,~,alldata] = xlsread('testdatacicEqual92k.xlsx');
